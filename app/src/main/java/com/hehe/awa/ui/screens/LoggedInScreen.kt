...
fun LoggedInScreen(
    ...
    profile: UserProfile?,
    onOpenProfile: () -> Unit,
    onLoadRequests: suspend () -> List<FriendRequest>,
    onLoadFriends: suspend () -> List<Friend>,
    onCreateRequest: suspend (String) -> UpdateResult,
    onAcceptRequest: suspend (String) -> UpdateResult,
    onRejectRequest: suspend (String) -> UpdateResult,
    onGetUserName: suspend (String) -> String?,
) {
    ...
    var requestUserNames by remember { mutableStateOf<Map<String, String>>(emptyMap()) }
    ...
    suspend fun loadData() {
        ...
        friends = onLoadFriends()
        
        val namesMap = mutableMapOf<String, String>()
        requests.forEach { request ->
            val otherUid = if (request.fromUid == user.uid) request.toUid else request.fromUid
            if (!namesMap.containsKey(otherUid)) {
                onGetUserName(otherUid)?.let { userName ->
                    namesMap[otherUid] = userName
                }
            }
        }
        requestUserNames = namesMap
    }
                ...
                if (requests.isNotEmpty()) {
                    ...
                    items(outgoingRequests) { request ->
                        ...
                        OutgoingRequestItem(
                            ...
                            onReject = {
                                scope.launch {
                                    val result = onRejectRequest(request.id)
                    ...
                    items(incomingRequests) { request ->
                        ...
                        IncomingRequestItem(
                            ...
                            onAccept = {
                                scope.launch {
                                    val result = onAcceptRequest(request.id)
                                    if (result is UpdateResult.Success) {
                                        loadData()
                                    }
                                }
            ...
            onConfirm = { tag ->
                scope.launch {
                    ...
                    if (toUid == null) {
                        ...
                        val result = onCreateRequest(toUid)
                        if (result is UpdateResult.Success) {
                            showAddDialog = false
                            loadData()
                        } else {
                            snackbarHostState.showSnackbar(
                                (result as? UpdateResult.Error)?.message ?: "Error"
                            )
                        }
                    }
                }
